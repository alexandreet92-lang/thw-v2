...
const DISTANCE_ORDER = ['50m', '100m', '200m', '400m', '800m', '1500m', '1km', '2km', '3.8km', '10km']
...
export function RecordsNatation({ personalRecords, raceResults }: Props) {
  ...
  const bestByYearDist = useMemo(() => {
    const map = new Map<string, Map<number, PersonalRecord>>()
    for (const r of personalRecords) {
      ...
      if (!existing) {
        ...
        const newS = parseSeconds(r.performance, r.performance_unit)
        const oldS = parseSeconds(existing.performance, existing.performance_unit)
        if (newS > 0 && (oldS === 0 || newS < oldS)) map.get(key)!.set(yr, r)
      }
    }
    return map
  }, [personalRecords])
  ...
  if (personalRecords.length === 0 && raceResults.length === 0) {
    return (
      <div className="text-center py-16 text-gray-400">
        <p className="text-sm">Aucun record natation enregistré.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-5">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left py-2 pr-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Distance</th>
                  {allYears.map((yr) => (
                    <th key={yr} className="text-right py-2 px-3 text-xs font-semibold text-cyan-500 uppercase tracking-wider">
                      {yr}
                    </th>
                  ))}
                  ...
                  return (
                    <tr key={dist} className="border-b border-gray-50 last:border-0">
                      ...
                      {allYears.map((yr) => {
                        ...
                        return (
                          <td key={yr} className="py-2.5 px-3 text-right">
                            <span className="font-semibold" style={{ color: isBest ? '#06B6D4' : '#374151' }}>
                      ...
                      <td className="py-2.5 pl-3 text-right">
                        {bestSecs > 0 ? (
                          <span className="font-bold text-cyan-500">{fmtTime(bestSecs.toString(), 'seconds')}</span>
      ...
      {raceResults.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
          <p className="text-sm font-semibold text-gray-800 mb-3">Résultats de compétition</p>
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right ml-4">
                    {r.dnf ? (
                      ...
                      <p className="text-sm font-bold text-cyan-500">{r.finish_time}</p>
                    )}
